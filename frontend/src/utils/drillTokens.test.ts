...
describe('drillTokens', () => {
  ...
  it('replaces TH drill tokens', () => {
    ...
    expect(
      replaceDrillTokens('Listen first. TH_THIN_MODEL then F_FIN_MODEL.')
    ).toBe('Listen first. th-in, thin then fff-in, fin.')
  })
  ...
  it('hides partial token fragments while assistant text is still streaming', () => {
    expect(normalizeStreamingDrillText('Listen first. R_R')).toBe(
      'Listen first. '
    )
    ...
    expect(
      normalizeStreamingDrillText('Listen first. R_RAH_MODEL. Try rah again.')
    ).toBe('Listen first. rrr-ah, rah. Try rah again.')
  })

  it('hides partial K/S/SH/TH token fragments while streaming', () => {
    expect(normalizeStreamingDrillText('Listen first. K_K')).toBe(
      'Listen first. '
    )
    expect(normalizeStreamingDrillText('Listen first. K_KEY_MODEL')).toBe(
      'Listen first. k-ee, key'
    )
    expect(normalizeStreamingDrillText('Listen first. SH_SH')).toBe(
      'Listen first. '
    )
    expect(normalizeStreamingDrillText('Listen first. SH_SHOE_MODEL')).toBe(
      'Listen first. sh-oo, shoe'
    )
    expect(normalizeStreamingDrillText('Listen first. TH_THOU')).toBe(
      'Listen first. '
    )
