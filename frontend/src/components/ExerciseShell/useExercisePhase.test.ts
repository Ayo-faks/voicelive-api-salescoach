...
import { INITIAL_PHASE_STATE, exercisePhaseReducer } from './useExercisePhase'
...
describe('exercisePhaseReducer — linear happy path', () => {
  ...
  it('ORIENT_DONE advances orient → expose', () => {
    const s = exercisePhaseReducer(stateAt({ phase: 'orient' }), {
      type: 'ORIENT_DONE',
    })
  ...
  it('ORIENT_DONE is a no-op outside orient', () => {
    const s = exercisePhaseReducer(stateAt({ phase: 'bridge' }), {
      type: 'ORIENT_DONE',
    })
  ...
  it('BRIDGE_DONE advances bridge → perform and stamps performStartedAt', () => {
    ...
    try {
      ...
      expect(s.performStartedAt).toBe(
        new Date('2026-04-19T10:00:00Z').getTime()
      )
    } finally {
      vi.useRealTimers()
    }
  ...
  it('BRIDGE_DONE is a no-op outside bridge', () => {
    const s = exercisePhaseReducer(stateAt({ phase: 'perform' }), {
      type: 'BRIDGE_DONE',
    })
    expect(s.phase).toBe('perform')
  ...
  it('PERFORM_DONE is a no-op outside perform', () => {
    const s = exercisePhaseReducer(stateAt({ phase: 'expose' }), {
      type: 'PERFORM_DONE',
    })
    expect(s.phase).toBe('expose')
  })
...
describe('exercisePhaseReducer — EXPOSE_INTERACT', () => {
  ...
  it('is idempotent after first interaction (same reference not required, state equal)', () => {
    const once = exercisePhaseReducer(stateAt({ phase: 'expose' }), {
      type: 'EXPOSE_INTERACT',
    })
...
describe('exercisePhaseReducer — variants (plan §D.2 items 12 & 13)', () => {
  it('SUPPRESS_BRIDGE jumps expose → perform with no bridge beat', () => {
    ...
    try {
      ...
      expect(s.phase).toBe('perform')
      expect(s.performStartedAt).toBe(
        new Date('2026-04-19T10:05:00Z').getTime()
      )
    } finally {
      vi.useRealTimers()
    }
  ...
  it('COLLAPSE_PERFORM is a no-op in perform/reinforce/orient', () => {
    expect(
      exercisePhaseReducer(stateAt({ phase: 'perform' }), {
        type: 'COLLAPSE_PERFORM',
      }).phase
    ).toBe('perform')
    expect(
      exercisePhaseReducer(stateAt({ phase: 'reinforce' }), {
        type: 'COLLAPSE_PERFORM',
      }).phase
    ).toBe('reinforce')
    expect(
      exercisePhaseReducer(stateAt({ phase: 'orient' }), {
        type: 'COLLAPSE_PERFORM',
      }).phase
    ).toBe('orient')
  })
