...
describe('TeacherMasteryDashboard', () => {
  it('surfaces the pilot dashboard goal metrics', () => {
    ...
    expect(screen.getAllByText('weakest sub-skills this week').length).toBeGreaterThan(0)
    expect(screen.getByText('7 students flagged for intervention')).toBeTruthy()
    expect(screen.getAllByText('3 proposed student facts awaiting approval').length).toBeGreaterThan(0)
    expect(screen.getByTestId('transparent-profile-entry')).toBeTruthy()
    expect(screen.getByRole('button', { name: 'Open student profile' })).toBeTruthy()
  })
  ...
  it('shows the roster for the selected class', () => {
    ...
    expect(screen.getByRole('button', { name: /Open profile for Tobi A\., Ratio mastery/ })).toBeTruthy()

    fireEvent.click(screen.getByRole('tab', { name: 'JSS1 A' }))

    expect(screen.getByText('Adaeze N.')).toBeTruthy()
    expect(screen.queryByRole('button', { name: /Open profile for Tobi A\., Ratio mastery/ })).toBeNull()
  ...
  it('shows a 1-2 week Pathfinder practice plan that the teacher approves', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockImplementation(input => {
            ...
            plans: [
              {
                ...
                plan: {
                  ...
                  rationale: 'Pathfinder proposes a small-group recovery plan based on low ratio mastery and high uncertainty.',
      ...
      if (url === '/api/learning/approvals/plan-jss2-ratio-recovery/approve') {
        return Promise.resolve(
          jsonResponse({ ok: true, plan_id: 'plan-jss2-ratio-recovery', action: 'approved' })
        )
      }
    ...
    expect(await screen.findByText('1-2 week practice plan awaiting teacher approval')).toBeTruthy()
    expect(screen.getByText('Pathfinder proposes; the teacher stays in charge.')).toBeTruthy()
    ...
    await waitFor(() => {
      expect(fetchMock.mock.calls.some(call => String(call[0]) === '/api/learning/approvals/plan-jss2-ratio-recovery/approve')).toBe(true)
    })
  })
  ...
  it('shows teacher-controlled memory facts that can be approved, edited, or rejected', async () => {
    ...
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockImplementation(input => {
      ...
      if (url === '/api/learning/student-facts/fact-1/edit-approve') {
        return Promise.resolve(jsonResponse({ ok: true, fact_id: 'fact-1', action: 'edited_approved' }))
      }

      if (url === '/api/learning/student-facts/fact-2/approve') {
        return Promise.resolve(jsonResponse({ ok: true, fact_id: 'fact-2', action: 'approved' }))
      }

      if (url === '/api/learning/student-facts/fact-3/reject') {
        return Promise.resolve(jsonResponse({ ok: true, fact_id: 'fact-3', action: 'rejected' }))
      }
    ...
    expect(screen.getByText(/personalization does not happen invisibly/i)).toBeTruthy()
    expect(screen.getByText('Needs worked examples before independent ratio practice')).toBeTruthy()
    expect(screen.getByText('Fraction bar visuals improve accuracy')).toBeTruthy()
    expect(screen.getByText('Prefers short voice prompts for review tasks')).toBeTruthy()

    await waitFor(() => {
      const approveButtons = screen.getAllByRole('button', { name: 'Approve' }) as HTMLButtonElement[]
    ...
    fireEvent.change(screen.getByLabelText('Edited memory fact for Tobi A.'), {
      target: { value: 'Needs two worked examples before independent ratio practice' },
    })
    ...
    await waitFor(() => {
      const editCall = fetchMock.mock.calls.find(call => String(call[0]) === '/api/learning/student-facts/fact-1/edit-approve')
      expect(editCall).toBeTruthy()
      expect(String((editCall?.[1] as RequestInit | undefined)?.body)).toContain('Needs two worked examples')
    })
    ...
    await waitFor(() => {
      expect(fetchMock.mock.calls.some(call => String(call[0]) === '/api/learning/student-facts/fact-2/approve')).toBe(true)
      expect(fetchMock.mock.calls.some(call => String(call[0]) === '/api/learning/student-facts/fact-3/reject')).toBe(true)
    })
  })
})