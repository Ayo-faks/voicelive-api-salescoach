...
def test_create_plan_builds_structured_draft():
    ...
    assert (
        runtime.calls[0]["planning_context"]["approved_child_memory"]["active_constraints"][0]["statement"]
        == "Keep cues short and specific."
    )
    assert plan["constraints"]["child_memory_snapshot"]["used_item_ids"] == ["memory-1", "memory-2"]
    assert (
        plan["constraints"]["child_memory_snapshot"]["used_items"][0]["statement"]
        == "Keep /r/ as an active therapy target."
    )
