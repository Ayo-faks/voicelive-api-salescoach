#!/usr/bin/env python3
"""Stage 1/2 builder for the Data Processing SS MCQ source bank.

Subject-parameterised twin of ``build_history_bank.py`` (the proven reference).
All items are *derived* (original-wording) Senior Secondary Data Processing
practice MCQs covering the WAEC/NECO SSCE Data Processing syllabus. They restate
widely taught Data Processing facts in our own words; no past-paper text is
reproduced verbatim and no PII is included. Every item is emitted with::
...
OUT = HERE / "data_processing-ss-mcq-v1.json"

VE, E, M, H, VH = -3.0, -1.5, 0.0, 1.5, 3.0

from data_processing_questions import TOPICS  # noqa: E402  (data lives in sibling module)
...
def build():
    ...
    for topic_key, (topic_display, pool) in TOPICS.items():
        ...
        for rec in pool:
            counter += 1
            iid = f"data_processing-mcq-ss3-{counter:03d}"
            ...
            skill_id = f"ss3.data_processing.{topic_key}.{rec['sub']}"
            item = {
                "lang": "en",
                "provenance": [
                    {
                        "source": "scrape:data_processing-ssce-derived",
                        "source_id": iid,
    ...
    bank = {
        "bank_id": "data_processing-ss-mcq-v1",
        "version": "1.0.0",
        "lang": "en",
        "subject": "data_processing",
        "taxonomy_version": "1.0.0",
