#!/usr/bin/env python3
"""Stage 1/2 builder for the Biology SS MCQ source bank.

Subject-parameterised twin of ``build_history_bank.py`` (the proven reference).
All items are *derived* (original-wording) Senior Secondary Biology
practice MCQs covering the WAEC/NECO SSCE Biology syllabus. They
...
OUT = HERE / "biology-ss-mcq-v1.json"

VE, E, M, H, VH = -3.0, -1.5, 0.0, 1.5, 3.0

from biology_questions import TOPICS  # noqa: E402  (data lives in sibling module)
...
def build():
    ...
    for topic_key, (topic_display, pool) in TOPICS.items():
        ...
        for rec in pool:
            counter += 1
            iid = f"biology-mcq-ss3-{counter:03d}"
            ...
            skill_id = f"ss3.biology.{topic_key}.{rec['sub']}"
            item = {
                "lang": "en",
                "provenance": [
                    {
                        "source": "scrape:biology-ssce-derived",
                        "source_id": iid,
