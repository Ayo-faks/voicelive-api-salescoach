#!/usr/bin/env python3
"""Stage 1/2 builder for the Literature-in-English SS MCQ source bank.

Subject-parameterised twin of ``build_history_bank.py`` (itself a twin of the
proven ``build_government_bank.py`` reference). All items are *derived*
(original-wording) Senior Secondary Literature-in-English practice MCQs covering
the WAEC/NECO SSCE syllabus (elements of literature, drama, poetry, prose,
figures of speech, sound devices, characterisation, themes/appreciation, African
and non-African texts). They restate widely-taught literary concepts and durable
authorship/genre facts in our own words; no past-paper text or set-text passage
is reproduced verbatim and no PII is included. Every item is emitted with::
...
OUT = HERE / "literature-ss-mcq-v1.json"

VE, E, M, H, VH = -3.0, -1.5, 0.0, 1.5, 3.0

from literature_questions import TOPICS  # noqa: E402  (data lives in sibling module)
...
def build():
    ...
    for topic_key, (topic_display, pool) in TOPICS.items():
        ...
        for rec in pool:
            counter += 1
            iid = f"literature-mcq-ss3-{counter:03d}"
            ...
            skill_id = f"ss3.literature.{topic_key}.{rec['sub']}"
            item = {
                "lang": "en",
                "provenance": [
                    {
                        "source": "scrape:literature-ssce-derived",
                        "source_id": iid,
