#!/usr/bin/env python3
"""Stage 1/2 builder for the Computer Science SS MCQ source bank.

Subject-parameterised twin of ``build_history_bank.py`` (the proven reference).
All items are *derived* (original-wording) Senior Secondary Computer Science
practice MCQs covering the WAEC/NECO SSCE Computer Science / Computer Studies
syllabus. They restate widely taught facts and standard worked examples in our
own words; no past-paper text is reproduced verbatim and no PII is included.
Every item is emitted with::
...
OUT = HERE / "computer_science-ss-mcq-v1.json"

VE, E, M, H, VH = -3.0, -1.5, 0.0, 1.5, 3.0

from computer_science_questions import TOPICS  # noqa: E402  (data lives in sibling module)
...
def build():
    ...
    for topic_key, (topic_display, pool) in TOPICS.items():
        ...
        for rec in pool:
            counter += 1
            iid = f"computer_science-mcq-ss3-{counter:03d}"
            ...
            skill_id = f"ss3.computer_science.{topic_key}.{rec['sub']}"
            item = {
                "lang": "en",
                "provenance": [
                    {
                        "source": "scrape:computer_science-ssce-derived",
                        "source_id": iid,
    ...
    bank = {
        "bank_id": "computer_science-ss-mcq-v1",
        "version": "1.0.0",
        "lang": "en",
        "subject": "computer_science",
        "taxonomy_version": "1.0.0",
