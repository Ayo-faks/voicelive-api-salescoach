#!/usr/bin/env python3
"""Stage 1/2 builder for the History SS MCQ source bank.

Subject-parameterised twin of ``build_government_bank.py`` (the proven
reference). All items are *derived* (original-wording) Senior Secondary History
practice MCQs covering the WAEC/NECO SSCE History syllabus. They restate widely
taught Nigerian / West African / world history facts in our own words; no
...
OUT = HERE / "history-ss-mcq-v1.json"

VE, E, M, H, VH = -3.0, -1.5, 0.0, 1.5, 3.0

from history_questions import TOPICS  # noqa: E402  (data lives in sibling module)
...
def _reband_topic(pool):
    ...
    place. Identical algorithm to build_government_bank.py:_reband_topic.
    """
    floor = 3
...
def build():
    ...
    for topic_key, (topic_display, pool) in TOPICS.items():
        ...
        for rec in pool:
            counter += 1
            iid = f"history-mcq-ss3-{counter:03d}"
            ...
            skill_id = f"ss3.history.{topic_key}.{rec['sub']}"
            item = {
                "lang": "en",
                "provenance": [
                    {
                        "source": "scrape:history-ssce-derived",
                        "source_id": iid,
