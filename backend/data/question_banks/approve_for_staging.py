#!/usr/bin/env python3
"""Owner-approval + automated sanity pass for staging.

Honest record: this is the PROJECT OWNER's explicit approval for STAGING
testing, not an independent subject-lead/safeguarding review. Rights for the
underlying past papers remain uncleared. Items that fail the automated
consistency check are QUARANTINED (flags left false -> never promoted).
"""
...
APPROVER = "project-owner (staging test approval)"
...
def arithmetic_ok(prompt, correct_text):
    """Best-effort recompute for purely-numeric 'Evaluate/Find' prompts.
    Returns None if not safely evaluable, else True/False."""
    ...
    # Ambiguous fractional-power notation like 27^1/3 -> skip (can't trust precedence)
...
def verify_item(it):
    """Structural consistency. Returns (ok, reason)."""
    ...
    if opts and letter:
        ...
        # correct_answer must reference the marked option text
...
for fn in ("maths_jss3_ss3_v1.json", "english_jss3_ss3_v1.json"):
    ...
    for it in bank["items"]:
        ...
        if ok:
            ...
            meta["verification_status"] = "owner_approved_staging"
            meta["answer_verified"] = True
            meta["approved_by"] = APPROVER
            meta["approved_at"] = APPROVED_AT
            meta["approval_scope"] = "staging_test_only"
            meta.setdefault("licence_note", "rights uncleared; staging test only, not for production/learners")
            approved += 1
        else:
            ...
            quarantined += 1
    # Approve bank only if at least one item approved
    if approved:
