#!/usr/bin/env python3
"""Stage 1/2 builder for the Economics SS MCQ source bank.

Subject-parameterised twin of ``build_history_bank.py`` (the proven reference).
All items are *derived* (original-wording) Senior Secondary Economics practice
MCQs covering the WAEC/NECO SSCE Economics syllabus. They restate widely taught
economic concepts and standard worked calculations in our own words; no
...
OUT = HERE / "economics-ss-mcq-v1.json"

VE, E, M, H, VH = -3.0, -1.5, 0.0, 1.5, 3.0

from economics_questions import TOPICS  # noqa: E402  (data lives in sibling module)
...
def build():
    ...
    for topic_key, (topic_display, pool) in TOPICS.items():
        ...
        for rec in pool:
            counter += 1
            iid = f"economics-mcq-ss3-{counter:03d}"
            ...
            skill_id = f"ss3.economics.{topic_key}.{rec['sub']}"
            item = {
                "lang": "en",
                "provenance": [
                    {
                        "source": "scrape:economics-ssce-derived",
                        "source_id": iid,
