#!/usr/bin/env python3
"""Stage 1/2 builder for the Physics SS MCQ source bank.
...
Secondary Physics practice MCQs covering the WAEC/NECO SSCE Physics syllabus.
...
OUT = HERE / "physics-ss-mcq-v1.json"

VE, E, M, H, VH = -3.0, -1.5, 0.0, 1.5, 3.0

from physics_questions import TOPICS  # noqa: E402  (data lives in sibling module)
...
def build():
    ...
    for topic_key, (topic_display, pool) in TOPICS.items():
        ...
        for rec in pool:
            counter += 1
            iid = f"physics-mcq-ss3-{counter:03d}"
            ...
            skill_id = f"ss3.physics.{topic_key}.{rec['sub']}"
            item = {
                "lang": "en",
                "provenance": [
                    {
                        "source": "scrape:physics-ssce-derived",
                        "source_id": iid,
